...
async fn compute_dimensions(conn: &mut Connection) -> Result<(i32, i32), AppError> {
    ...
    let w = (out.rect.width as f32 * 0.30).round() as i32;
    let h = (out.rect.height as f32 * 0.40).round() as i32;
    Ok((w, h))
}

async fn apply_rules(conn: &mut Connection) -> Result<(), AppError> {
    let (w, h) = compute_dimensions(conn).await?;
